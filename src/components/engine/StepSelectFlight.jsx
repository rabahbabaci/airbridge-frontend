...
import { Plane, AlertCircle, CheckCircle2, Search, ArrowLeft, ArrowRight } from 'lucide-react';
...
export default function StepSelectFlight({
    flightOptions, selectedFlight, searching, searchError,
    inputMode, flightNumber, departureDate,
            ...
            {searching ? (
                ...
                <motion.div initial="hidden" animate="visible" className="space-y-3">
                    {flightOptions.map((f, i) => {
                        ...
                        return (
                            <motion.div key={i} custom={i + 2} variants={stagger}>
                                <button
                                    onClick={() => onFlightClick(f)} disabled={isDisabled}
                                    className={`w-full text-left rounded-2xl border-2 bg-card px-5 transition-all duration-200 ${
                                        isDisabled ? 'opacity-50 cursor-not-allowed border-border py-4' :
                                        isSelected ? 'border-primary bg-accent/50 shadow-md shadow-primary/10 py-4' :
                                        'border-border hover:border-muted-foreground/30 hover:shadow-sm py-4'
                                    }`}
                                ...
                                {/* Inline Continue button inside selected card */}
                                <AnimatePresence>
                                    {isSelected && !isDisabled && (
                                        <motion.div
                                        ...
                                        >
                                            <div className="bg-accent/50 border-2 border-t-0 border-primary rounded-b-2xl px-5 pt-5 pb-4"
                                                style={{ borderLeftWidth: '4px', borderLeftColor: 'hsl(var(--primary))' }}>
                                                <button onClick={onContinue}
                                                    className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-primary hover:bg-brand-accent text-primary-foreground text-sm font-semibold transition-all shadow-sm hover:shadow-md">
                                                    Continue
