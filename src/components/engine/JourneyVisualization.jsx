...
import { Home, Plane, Car, Train, Bus, User, Shield, Luggage, DoorOpen, CheckCircle2, Footprints } from 'lucide-react';
...
const stepMeta = {
    ...
    trainwalk: { icon: Footprints,   label: 'Walk to Train',  color: '#8b5cf6', glow: 'rgba(139,92,246,0.6)'  },
...
function StepNode({ stepId, time, dur, terminal, revealed, TransportIcon, stepNumber }) {
    ...
    // What to show as the primary value under the icon
    const primaryLabel = (() => {
        if (stepId === 'home') return time;          // just the leave time
        if (stepId === 'travel') return null;         // animated dots instead
        if (stepId === 'airport') return time;        // arrive time
        if (stepId === 'gate') return time;           // boarding time
        return time;                                  // all others: arrival time at that step
    })();

    // Sub-label (duration / context)
    const subLabel = (() => {
        if (stepId === 'home') return null;            // no duration under Leave Home
        if (stepId === 'travel') return dur;           // "63 min" shown via animated label on bar
    ...
    return (
        ...
        >
            ...
            {/* Primary value (time or animated dots for travel) */}
            {stepId === 'travel' ? (
                revealed ? (
...
function HBar({ revealed, color, label, reverse, duration = 0.9 }) {
    return (
        <div className="flex flex-col flex-1 px-1 self-start" style={{ minWidth: 20, marginTop: 24 }}>
            {/* Label above the bar */}
...
export default function JourneyVisualization({ locked, steps, transport, profile, confidenceColorMap, onReady }) {
    ...
    const visibleSteps = steps.filter(s => s.id !== 'baggage' || s.visible).filter(s => s.id !== 'trainwalk' || s.visible);
    ...
    // Helper to get step dur label for bar
    const getBarLabel = (stepId) => {
                        ...
                        <div className="rounded-2xl px-4 pt-5 pb-6 overflow-hidden"
                            ...
                            <div className="flex items-start">
                                {row1.map((s, i) => {
                                    ...
                                    // Bar label: show duration of the CURRENT segment being traversed to reach the next node
                                    const barLabel = nextS ? getBarLabel(nextS.id === 'travel' ? 'travel' : nextS.id === 'trainwalk' ? 'trainwalk' : nextS.id) : null;
                                    return (
                            ...
                            <div className="flex items-start">
                                {row2.map((s, i) => {
                                    ...
                                    const barLabel = nextS ? getBarLabel(nextS.id) : null;
                                    return (
                                        <React.Fragment key={s.id}>
                                            <StepNode
